...
public class StakeHoldersProtocol {
	...
	public Message processInput(Message message, Repository repo) {
		...
		try {
			method = repo.getClass().getDeclaredMethod(message.getFunctionName(),argsClass);
		} catch (NoSuchMethodException | SecurityException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			objectReturn = method.invoke(repo,message.getArgs());
