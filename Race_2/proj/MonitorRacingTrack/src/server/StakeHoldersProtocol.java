...
public class StakeHoldersProtocol {
	...
	public String processInput(String payload, MonitorRacingTrack mRacingTrack) {
		...
		if (method.getReturnType().isArray()) {
		        System.out.println("ISARRAY "+method.getReturnType().getComponentType().equals(Integer.TYPE));
		        if(method.getReturnType().getComponentType().equals(Integer.TYPE)) {
		        	...
		        	for(int i=0;i<tempIntArray.length;i++) {
		        		System.out.println("tempIntArray["+i+"]"+tempIntArray[i]);
		        		returnFunctionPayload+=Integer.toString(tempIntArray[i]);
		        		if(i<tempIntArray.length-1) {
		        			returnFunctionPayload+=",";
		    			}
		        	}
		        	System.out.println("final string->"+returnFunctionPayload);
		        }
