...
public class StakeHoldersProtocol {
	...
	public Message processInput(Message message, MonitorRacingTrack mRacingTrack) {
		...
		try {
			method = mRacingTrack.getClass().getDeclaredMethod(message.getFunctionName(),argsClass);
		} catch (NoSuchMethodException | SecurityException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			objectReturn = method.invoke(mRacingTrack,message.getArgs());
