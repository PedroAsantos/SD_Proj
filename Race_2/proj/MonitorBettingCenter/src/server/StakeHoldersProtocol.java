...
public class StakeHoldersProtocol {
	...
	public Message processInput(Message message, MonitorBettingCenter mBettingCenter) {
		...
		try {
			method = mBettingCenter.getClass().getDeclaredMethod(message.getFunctionName(),argsClass);
		} catch (NoSuchMethodException | SecurityException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			objectReturn = method.invoke(mBettingCenter,message.getArgs());
