...
public class StakeHoldersProtocol {
	...
	public Message processInput(Message message, MonitorControlCenter mControlCenter) {
		...
		try {
			method = mControlCenter.getClass().getDeclaredMethod(message.getFunctionName(),argsClass);
		} catch (NoSuchMethodException | SecurityException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			objectReturn = method.invoke(mControlCenter,message.getArgs());
